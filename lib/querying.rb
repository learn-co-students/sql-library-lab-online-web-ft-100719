...
def select_series_title_with_most_human_characters
  "SElECT series.title FROM books INNER JOIN series ON series.id = books.series_id INNER JOIN character_books ON books.id = character_books.book_id INNER JOIN characters ON characters.id = character_books.character_id GROUP BY characters.species ORDER BY COUNT(characters.species) LIMIT 1;"
end
